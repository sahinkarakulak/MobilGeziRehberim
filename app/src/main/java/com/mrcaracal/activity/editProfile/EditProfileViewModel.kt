...
class EditProfileViewModel: ViewModel() {
    ...
    var editProfileViewState : MutableLiveData<EditProfileViewState> = MutableLiveData<EditProfileViewState>()
            ...
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    if (documentSnapshot.exists()) {
                        editProfileViewState.value = EditProfileViewState.GetFirebaseDocValUserName(documentSnapshot = documentSnapshot, firebaseDocValUserName = FIREBASE_DOC_VAL_USERNAME)
                        editProfileViewState.value = EditProfileViewState.GetFirebaseDocValBio(documentSnapshot = documentSnapshot, firebaseDocValBio = FIREBASE_DOC_VAL_BIO)
                        editProfileViewState.value = EditProfileViewState.PicassoPross(documentSnapshot = documentSnapshot, firebaseDocValueUserPic = FIREBASE_DOC_VAL_USERPIC)
                    }
                }
            }
    }
    ...
    fun uploadImage(contentResolver: ContentResolver) {
        ...
        uploadTask.continueWithTask { task ->
            ...
            editProfileViewState.value = EditProfileViewState.ShowExceptionErrorMessage(exception = exception)
        }
    }
}
...
sealed class EditProfileViewState {
    ...
    data class GetFirebaseDocValUserName(val documentSnapshot: DocumentSnapshot, val firebaseDocValUserName: String): EditProfileViewState()
    data class GetFirebaseDocValBio(val documentSnapshot: DocumentSnapshot, val firebaseDocValBio: String): EditProfileViewState()
    data class PicassoPross(val documentSnapshot: DocumentSnapshot, val firebaseDocValueUserPic: String): EditProfileViewState()
